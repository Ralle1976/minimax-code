...
To develop MCode or run this source checkout, you need Git, **Node.js 22.19+ (22.x), 24.2+ (24.x), 25, or 26**, and **pnpm 9.12.0**. On Windows, keep the checkout on a local NTFS volume and outside cloud-synced folders; the preflight command below checks the volume before pnpm creates workspace links. If `pnpm install` stops while linking workspace packages with `ENOENT … forceSymlink`, the volume cannot provide NTFS junctions — the [installation guide](docs/installation.md) documents the failure signature and recovery.
```bash
